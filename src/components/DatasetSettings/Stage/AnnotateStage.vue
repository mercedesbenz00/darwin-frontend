<template>
  <stage-template
    class="annotate"
    :class="{
      'annotate--rates': hasSampling,
    }"
  >
    <template #header>
      <annotate-round-icon class="annotate__header__icon" />
      <stage-title
        :value="name"
        @change="updateName"
      />
      ...
      <sampling-rate
        v-if="hasSampling"
        class="annotate__base-sampling rates-asside"
    ...
    <template #body>
      <div class="annotate__body rates-container">
        <div class="rates-row">
          <input-field
            v-model="search"
          ...
          <sampling-info
            v-if="hasSampling"
            class="annotate__search-row__info"
          />
        </div>
        <div class="annotate__body__list scroll">
          <div class="annotate__body__list__item rates-row scroll__row">
          ...
          <div
            v-for="actor of filteredSelectionData"
            :key="actor.member.user_id"
            class="annotate__body__list__item rates-row scroll__row"
          >
            <annotator-score-popover :actor="actor">
              <annotator-toggle
                :class="{ 'annotate__body__list__item__toggle--fade': !manualAssignment }"
                :member="actor.member"
                :selected="actor.selected"
                :sampling-rate="actor.samplingRate"
                @select="toggleStageAssignee(actor)"
              />
            </annotator-score-popover>
            <sampling-rate
              v-if="hasSampling"
              class="annotate__annotator-sampling"
              :value="actor.samplingRate"
              :disabled="!(manualAssignment && actor.selected)"
              @change="rate => updateAssigneeSamplingRate(actor, rate)"
            />
          </div>
        </div>
      </div>
...
export default class AnnotateStage extends Vue {
  /**
  ...
  assignableTo: AnnotateStageTemplatePayload['metadata']['assignable_to'] = 'anyone'
  ...
  setAssignableTo (value: AnnotateStageTemplatePayload['metadata']['assignable_to']): void {
  ...
  emitChange (): void {
    ...
    const updatedStage: AnnotateStageTemplatePayload = {
      ...stage,
      metadata: {
        ...stage.metadata,
...
.annotate__header__icon {
  height: 20px;
  width: 20px;
}

.annotate .annotate__body {
 @include scroll;
}

.annotate__body__list__item {
  width: 100%;
  display: grid;
...
.annotate__body__list__item__toggle--fade {
  opacity: 0.5;
}

.annotate__search-row__info {
  margin-left: 5px;
}
</style>
