...
import CheckBoxV2 from './CheckBox.vue'

const stories = {
  title: 'Common/CheckBoxV2',
  ...
  argTypes: {
    ...
    label: { control: 'text' },
    name: { control: 'text' },
    size: { control: { type: 'radio', options: ['default', 'large'] } },
...
export const Interactive = (args: Args, { argTypes }: any) => Vue.extend({
  ...
  template:
    '<check-box-v2 v-bind="$props" :value="checked" v-on="actions" @change="toggleCheck" />'
})
...
export const LimitedContainerWidth = (args: Args, { argTypes }: StoryConfig) => Vue.extend({
  ...
  template: `
    <div :style=style>
      <check-box-v2 v-bind="$props" v-on="actions"/>
    </div>
  `
})
