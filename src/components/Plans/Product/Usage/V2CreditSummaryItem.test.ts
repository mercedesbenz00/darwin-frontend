...
import V2CreditSummaryItem from '@/components/Plans/Product/Usage/V2CreditSummaryItem.vue'
import { BillingInfoPayload, ProductType } from '@/store/modules/billing/types'
import { CreditUsagePayloadV2, TeamPayload } from '@/store/types'
...
class Model {
  ...
  get bar (): { wrapper: Wrapper<Vue> } {
    const bar = this.wrapper.find('v2-credit-bar-stub')
    return {
      wrapper: bar
    }
  }
...
const itMatchesSnapshot = (): void => it('matches snapshot', () => {
  const wrapper = shallowMount(V2CreditSummaryItem, { localVue, propsData, store, stubs })
...
describe('when team is partner with clients', () => {
  ...
  beforeEach(() => {
    ...
    const partnerUsage = buildCreditUsagePayloadV2({
      team_id: 1,
      clients: [
        buildCreditUsagePayloadV2({ team_id: 11 }),
        buildCreditUsagePayloadV2({ team_id: 12 })
      ]
    })
...
describe('when team is client on own page', () => {
  beforeEach(() => {
    propsData = {
      ...
      billingInfo: buildBillingInfoPayload({ }),
      creditUsage: buildCreditUsagePayloadV2({ team_id: 1 })
    }
  })
