...
import { useAuth, useRoute, useStore } from '@/composables'
import annotatorsRoute from '@/router/annotatorsRoute'
...
export default defineComponent({
  ...
  setup (props) {
    ...
    const routeLocation: Location = { path: annotatorsRoute.path }
    ...
    const team = computed(() => state.team.currentTeam)

    const label = computed((): string => {
      return isAuthorized('view_full_datasets', { subject: 'team', resource: team })
        ? 'Annotators'
        : 'Statistics'
    })
