<template>
  <form @submit.prevent="onSubmit">
    ...
    <div class="storage__field-label-container">
      <div>
        <span class="storage__field-label">S3 Bucket</span>
        <span class="field-required">*</span>
      </div>
      <info title="Bucket (Required)">
        Name of the storage bucket / storage account name.
        In case of AWS S3, please don't use full ARN, just the name of the bucket.
      </info>
    </div>
    <input-field
      id="storageBucket"
    ...
    />
    <div class="storage__field-label-container">
      <span class="storage__field-label">S3 Prefix</span>
      <info title="Prefix (Optional)">
        Prefix for all Darwin-created files (like thumbnails).
        It cannot start nor end with "/". Examples: "v7labs", "mycontainer/darwin".
      </info>
    </div>
    <input-field
      v-model="prefix"
    ...
    <div class="storage__field-label-container">
      <div>
        <span class="storage__field-label">S3 Region</span>
        <span class="field-required">*</span>
      </div>
      <info title="Region (Required)">
        Region identifier for the S3 bucket.
      </info>
    </div>
    <s3-region-select
      v-model="region"
...
export default class AwsStorageForm extends Vue {
  ...
  validateForm (): boolean {
    ...
    if (isEmpty(this.region.trim())) {
      this.error.region = 'Region cannot be empty'
    }
  ...
  onSubmit (): void {
    if (!this.validateForm()) {
      this.$ga.event('aws_storage', 'submit', 'failure_form_invalid')
      return
    }
