...
beforeEach(() => {
  ...
  storagePayload = buildStoragePayload({ provider: 'gcp' })
  propsData = { value: storagePayload }
})

it('matches snaphsot', () => {
  const wrapper = shallowMount(GcpStorageForm, { localVue, mocks, store, propsData })
...
describe('validation', () => {
  ...
  it('shows error when name is empty', async () => {
    ...
    const wrapper = shallowMount(GcpStorageForm, { localVue, data, mocks, store, propsData })
    await triggerRootStub(wrapper, 'submit')
    expect(wrapper.find(model.nameInput).props('error')).toEqual('Storage name cannot be empty')
    expect(mocks.$ga.event).toBeCalledWith('gcp_storage', 'submit', 'failure_form_invalid')
  })
