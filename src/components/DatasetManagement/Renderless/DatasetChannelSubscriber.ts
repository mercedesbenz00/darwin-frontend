...
export default class DatasetChannelSubscriber extends Vue {
  ...
  async onTopic (topic: string, oldTopic: string | null) {
    if (oldTopic) { await this.$store.dispatch('dataset/leaveChannel', { topic: oldTopic }) }
    if (!this.authenticated) { return }
    if (topic) {
      await this.$store.dispatch('dataset/joinChannel', { topic })
      this.$once('hook:beforeDestroy', () => {
        this.$store.dispatch('dataset/leaveChannel', { topic })
      })
    }
  }
