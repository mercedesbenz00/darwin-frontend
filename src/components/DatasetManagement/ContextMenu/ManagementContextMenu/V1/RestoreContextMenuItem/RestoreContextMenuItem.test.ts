...
import RestoreContextMenuItem from './RestoreContextMenuItem.vue'
...
it('matches snapshot', () => {
  const wrapper = shallowMount(RestoreContextMenuItem, { localVue, mocks, propsData, store })
...
it('dispatches toast if user is not allowed to archive', async () => {
  ...
  expect(store.dispatch).toHaveBeenCalledWith('toast/warning', {
    content: errorsByCode.DATASET_IMAGE_RESTORE_NOT_AUTHORIZED
  })
})
...
it('dispatches action if user allowed to archive', async () => {
  ...
  expect(store.dispatch).toHaveBeenCalledWith('dataset/restoreDatasetItems', {
