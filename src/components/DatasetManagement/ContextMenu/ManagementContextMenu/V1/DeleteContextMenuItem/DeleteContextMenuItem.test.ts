...
import DeleteContextMenuItem from './DeleteContextMenuItem.vue'
...
class ComponentModel extends Model {
  ...
  confirmModal (): Vue {
    return this.wrapper.find('delete-confirmation-dialog-stub').vm.$emit('confirmed')
  }
}

it('matches snapshot', () => {
  const wrapper = shallowMount(DeleteContextMenuItem, { localVue, mocks, propsData, store })
...
it('dispatches action if user allowed to archive', async () => {
  ...
  expect(store.dispatch).toHaveBeenCalledWith('dataset/deleteDatasetItems', {
