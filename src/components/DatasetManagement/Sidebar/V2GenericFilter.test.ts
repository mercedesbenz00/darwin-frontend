...
beforeEach(() => {
  ...
  store.commit('dataset/SET_V2_DATASET_FOLDERS', { datasetId: dataset.id, folders })
  store.commit('dataset/SET_DATASET_ITEM_FILENAMES', filenames)
...
it('matches snapshot when loading', () => {
  store.commit('loading/SET_ACTION_LOADING_STATUS', {
    key: 'dataset/loadV2DatasetFolders',
    status: LoadingStatus.Loading
  })
  store.commit('loading/SET_ACTION_LOADING_STATUS', {
    key: 'dataset/searchFilenames',
    status: LoadingStatus.Loading
  })
  const wrapper = shallowMount(GenericFilter, { localVue, mocks, propsData, store })
  expect(wrapper).toMatchSnapshot()
...
describe('when search text changes', () => {
  it('dispatches action to search filenames', async () => {
    ...
    expect(store.dispatch).toBeCalledWith(
      ...
      { search: 'keyword', datasetId: v7.id, teamSlug: 'team' }
    )
  })
})
