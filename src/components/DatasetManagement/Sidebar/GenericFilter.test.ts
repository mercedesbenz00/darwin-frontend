...
import GenericFilter from '@/components/DatasetManagement/Sidebar/GenericFilter.vue'
import {
...
beforeEach(() => {
  ...
  store.commit('dataset/SET_DATASET_FOLDERS', { datasetId: dataset.id, folders })
...
describe('when search text changes', () => {
  it('dispatches action to search filenames', async () => {
    ...
    expect(store.dispatch).toBeCalledWith(
      ...
      { search: 'keyword', datasetId: v7.id }
    )
  })
})
