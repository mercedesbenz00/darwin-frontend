...
export default class DataSidebarGenericFilter extends Vue {
  ...
  @Getter('currentPathFoldersV2', { namespace: 'dataset' })
  ...
  get allFolders () {
    return this.folderEnabled
      ? this.folders.filter(folder =>
        (this.path === '/' && folder.path !== '/') || folder.path.includes(`${this.path}/`)
      )
      : []
  }
  ...
  searchFilenames (search: string) {
    this.$store.dispatch(
      'dataset/searchFilenamesThrottledV2',
      { search, datasetId: this.dataset.id, teamSlug: this.dataset.team_slug }
    )
  }
  ...
  updateFilter (updates:
    ...
    {
      positiveFolders: V2DatasetFolderPayload[],
      negativeFolders: V2DatasetFolderPayload[]
    } |
    {
