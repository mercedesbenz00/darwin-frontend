...
describe('when the modal is shown', () => {
  it('dispatch actions to load data', () => {
    const wrapper = shallowMount(NewExportDialog, { localVue, mocks, store })
    const vm = wrapper.vm as any
    vm.show()

    expect(store.dispatch).toBeCalledWith('dataset/getExportCompleteCount', { datasetSlug: 'sfh', teamSlug: 'v7' })
...
describe('when no images are completed', () => {
  ...
  it('matches snapshot', () => {
    const wrapper = shallowMount(NewExportDialog, { localVue, store, mocks })
    expect(wrapper).toMatchSnapshot()
  })
