...
import { inputTagsToBadges, toTagItem, initializeTag } from './utils'
...
export default defineComponent({
  ...
  setup (props) {
    ...
    const currentTags = computed((): InputTag[] => {
      if (activeView.value.fileManager.isProcessedAsVideo) {
        return (tags.value || [])
          .filter(tag => !tag.segments || (tag.segments || [])
            .some(range =>
              // assumes segment second is `null` it's should be full video wide
              activeView.value.currentFrameIndex >= range[0] &&
              (range[1] === null || activeView.value.currentFrameIndex < range[1])
            )
          )
      }
      return tags.value
    ...
    const tagAnnotations = computed((): Annotation[] => {
      ...
      return activeView.value.annotationManager.tagAnnotations
    })
    ...
    const onCreateTag = async (
      ...
      if (annotationClass) {
        ...
        await activeView.value.annotationManager.updateAnnotation(tagAnnotation)
      }
    ...
    const onDeleteTag = async (id: string): Promise<void> => {
      ...
      const tagAnnotation = (tagAnnotations.value || [])
        .find(a => a.id === tag.id)!
      await activeView.value.annotationManager.deleteAnnotationAction(tagAnnotation)
      tag.loading = false
    }
