...
import { tagFromAttribute } from '@/engineV2/plugins/attributes/utils'
...
export default class AttributesInput extends Vue {
  ...
  async saveAllTags (newTags: InputTag[]): Promise<void> {
    ...
    const annotation = this.editor.activeView.annotationManager
      .initializeSubAnnotation('attributes', this.masterAnnotation, data)
    if (!annotation) { return }
    const action = addOrUpdateSubAnnotation(
      this.editor.activeView,
      annotation,
