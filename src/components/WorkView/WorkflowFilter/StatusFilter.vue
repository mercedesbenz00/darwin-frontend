...
const ICONS: { [k in FilterableStatus]: string } = {
  complete: '/static/imgs/image-status/completed.svg',
  new: '/static/imgs/image-status/new.svg',
  annotate: '/static/imgs/image-status/annotate.svg',
  review: '/static/imgs/image-status/review.svg',
  webhook: '/static/imgs/image-status/webhook.svg'
}
...
export default class WorkflowSidebarStatusFilter extends Vue {
  ...
  get commentOption (): StatusFilterItemType {
    const { counts } = this
    return {
      id: 'commented',
      label: 'Commented',
   ...
   */
  get filterStatusOptions (): StatusFilterItemType[] {
    const { counts } = this

    const options: StatusFilterItemType[] = STATUSES.map(key => {
      return {
        id: key,
        label: LABELS[key],
        icon: ICONS[key],
