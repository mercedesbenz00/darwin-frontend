...
export default defineComponent({
  ...
  setup (props) {
    ...
    const allClasses = computed(() => props.editor.activeView.annotationManager.annotationClasses)
    ...
    const classesForTool = computed(() => {
      ...
      return datasetClasses.value.filter(
        c => {
          if (annotationType.value === 'polygon') {
            return annotationType.value && (c.annotation_types.includes(annotationType.value) 
            || c.annotation_types.includes('mask'))
          }
          return annotationType.value && c.annotation_types.includes(annotationType.value)
        }
      )
