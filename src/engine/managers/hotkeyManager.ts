...
import { addAnnotationAction } from '@/engine/actions'
...
export class HotkeyManager {
  ...
  public registerDefaultHotkeyListeners (): void {
    ...
    this.$on({ code: 'KeyH' }, (event) => {
      event.preventDefault()
      event.shiftKey ? editor.toggleSubAnnotations() : editor.toggleAnnotations()
    })

    this.$on({ code: 'KeyM', metaKey: true }, (event) => {
      event.preventDefault()
      editor.toggleMeasures()
    })
    ...
    this.$on({ code: 'ArrowUp' }, (event) => {
      event.preventDefault()
      editor.moveSelectedAnnotation(new Point<'Image'>({ x: 0, y: event.shiftKey ? -5 : -1 }))
    })
    ...
    this.$on({ code: 'ArrowLeft' }, (event) => {
      event.preventDefault()
      editor.moveSelectedAnnotation(new Point<'Image'>({ x: event.shiftKey ? -5 : -1, y: 0 }))
    })
    ...
    this.$on({ key: 'a', metaKey: true }, (event) => {
      ...
      if (selectedAnnotation && hasAttributesSubAnnotation(selectedAnnotation)) {
        editor.activateTool('attributes_tool', { sub: { master: selectedAnnotation } })
      }
    })
    ...
    this.$on(
      ...
      () => editor.activeView.performMoveAction(),
      { type: 'keyup' }
    )
    this.$on(
      {},
  ...
  registerTabBehavior (): void {
    ...
    this.$on({ code: 'Tab' }, (event) => {
      ...
      if (event.shiftKey) {
        const previousAnnotation = editor.selectPreviousAnnotation()
        if (previousAnnotation) {
          this.editor.zoomToAnnotation(previousAnnotation)
        }
      } else {
        const nextAnnotation = editor.selectNextAnnotation()
        if (nextAnnotation) {
          this.editor.zoomToAnnotation(nextAnnotation)
        }
      }
    }, { type: 'keyup' })
  }
  ...
  private selectAnnotationClass (event: KeyboardEvent): void {
    ...
    const { annotationClasses } = this.editor.activeView
    ...
    if (this.editor.maybeChangeSelectedAnnotationClass(annotationClass)) { return }

    const { name: typeName } = this.editor.getMainAnnotationTypeForClass(annotationClass)
  ...
  public async handleTagAnnotationClass (tagAnnotationClass: AnnotationClass): Promise<void> {
    const { actionManager } = this.editor
    const { activeView } = this.editor
    const { annotations } = activeView

    // if the tag exists and the content is a video or a DICOM image
    // we need to check in the selected frame the tag exists:
    // - if yes we remove it
    // - if not we add it to the current frame segment
    if (activeView.loadedVideo) {
      // check if the currently selected video frame already
      // contains that tag
      const tagWithinCurrentFrameSegment = (annotations || [])
        // a frame with the same classId already exists
        .filter(({ classId }) => classId === tagAnnotationClass.id)
        // and it intersect witin the current selected frame segment
        .filter(({ data: { segments } }) => !segments ||
          (segments || [])
            .some((range: [number, number]) => {
              const intersectCurrentFrameSegment = activeView.currentFrameIndex >= range[0] &&
                activeView.currentFrameIndex < range[1]
              return intersectCurrentFrameSegment
            })
        )
      // If annotation for this tag annotation class exists, remove it.
      // If not, add a new annotation.
      if ((tagWithinCurrentFrameSegment || []).length > 0) {
        await activeView.removeAnnotation(tagWithinCurrentFrameSegment[0])
        return
      }
    // If the tag exists and the content is an image we simply remove it
    // from the whole image
      ...
      if (existingTagAnnotation) {
        await activeView.removeAnnotation(existingTagAnnotation)
        return
      }
    }
    ...
    const { loadedVideo, store } = activeView
    let annotation: Annotation | null
    if (!loadedVideo) {
      annotation = this.editor.initializeAnnotation({
        type: 'tag',
        actors: [actorPayload],
    ...
    actionManager.do(addAnnotationAction(this.editor, annotation))
  }
