...
import { Keybinding, keybindingMatch } from '@/engine/keybinding'
import { AnnotationOverlayer } from '@/engine/managers/overlayManager'
import { AnnotationTypeSerializer } from '@/engine/managers/serializerManager'
import { Tool } from '@/engine/managers/toolManager'
import { MainAnnotationTypeRenderer } from '@/engine/models'
import attributesPlugin from '@/engine/plugins/attributes'
import boundingBox from '@/engine/plugins/boundingBox'
import boundingBox3d from '@/engine/plugins/boundingBox3d'
import brush from '@/engine/plugins/brush'
import clicker from '@/engine/plugins/click'
import clipboardPlugin from '@/engine/plugins/clipboard'
import commentatorPlugin from '@/engine/plugins/commentator'
import directionalVector from '@/engine/plugins/directionalVector'
import editPlugin from '@/engine/plugins/edit'
import ellipse from '@/engine/plugins/ellipse'
import fieldPlugin from '@/engine/plugins/field'
import inferencePlugin from '@/engine/plugins/inference'
import instanceIDPlugin from '@/engine/plugins/instanceId'
import keypoint from '@/engine/plugins/keypoint'
import link from '@/engine/plugins/link'
import magnifierGlass from '@/engine/plugins/magnifierGlass'
import measuresPlugin from '@/engine/plugins/measures'
import polygon from '@/engine/plugins/polygon'
import polyline from '@/engine/plugins/polyline'
import saveAnnotations from '@/engine/plugins/saveAnnotations'
import selectPlugin from '@/engine/plugins/select'
import skeleton from '@/engine/plugins/skeleton'
import tablePlugin from '@/engine/plugins/table'
import tag from '@/engine/plugins/tag'
import textPlugin from '@/engine/plugins/text'
import v7Look from '@/engine/plugins/v7'
import videoPlayerPlugin from '@/engine/plugins/videoPlayer'
import windowLevelPlugin, { WindowLevelPlugin } from '@/engine/plugins/windowLevel'
import zoomPlugin from '@/engine/plugins/zoom'
import zoomCorrection from '@/engine/plugins/zoomCorrection'
import { AnnotationTypeName, DatasetPayload } from '@/store/types'

import { MeasureOverlayer } from './measureManager'
  ...
  toolOptions: [
    ...
    },
    {
      ...
      isVisible: (editor?: Editor): boolean => {
        ...
        const { selectedAnnotation } = editor
        if (!selectedAnnotation) { return false }
        return selectedAnnotation.type === 'polygon' && !editor.loadedVideo
      }
    },
    {
      id: 'polygon_subtract',
...
const FieldToolConfig: ToolConfig = {
  name: 'field_tool',
  toolTip: `
    <b>Field Tool <span class="tooltip__hotkey">G</span></b><br>
  ...
  icon: 'field',
  keybindings: [
    { keys: ['g'], action: 'field_tool.activate' },
    {
      keys: ['Escape'],
...
const stringConfig: PluginConfig = {
  ...
  context: undefined,
  keybindings: [],
  tools: [{ ...FieldToolConfig, priority: 9 }]
}
...
const tableConfig: PluginConfig = {
  ...
  context: undefined,
  keybindings: [],
  tools: [{ ...tableToolConfig, priority: 10 }]
}

export const sharedBackendPlugins: PluginConfig[] = [
  // sidebar
  ...
  { ...commentatorConfig, tools: [{ ...commentatorToolConfig, priority: 98 }] },
  { ...zoomConfig, tools: [{ ...zoomToolConfig, priority: 99 }] },
...
export class PluginManager {
  ...
  public pluginsForTutorial (): PluginConfig[] {
    return sharedBackendPlugins.filter((pluginConfig) => pluginConfig.name !== 'commentator')
  }
