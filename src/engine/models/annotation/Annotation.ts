...
import { enableInterpolateByDefault } from '@/engine/managers/renderManager'
import { AnnotationTypeRenderer, MainAnnotationTypeRenderer } from '@/engine/models'
import { IView, VideoAnnotationDataPayload } from '@/engine/models/views/types'
import { BoundingBox } from '@/engine/plugins/boundingBox/types'
import { String, StringSource } from '@/engine/plugins/field/types'
import { Polygon } from '@/engine/plugins/polygon/types'
import { Table } from '@/engine/plugins/table/types'
import { Text } from '@/engine/plugins/text/types'
import { BBox, getBBox } from '@/engine/utils'
...
const buildCumulativeBox = (sources: StringSource[], view: IView): BoundingBox => {
  ...
  for (const source of sources) {
    const sourceAnnotation = view.editor.mainAnnotations.find(a => a.id === source.id)
...
export class Annotation {
  ...
  private initializeText (): void {
    ...
    for (const source of data.sources) {
      const sourceAnnotation = this.view.mainAnnotations.find(a => a.id === source.id)
      if (!sourceAnnotation) { continue }

      if (sourceAnnotation.isVideoAnnotation()) {
        const sourceSubs = this.view.inferVideoSubAnnotationDataOnly(sourceAnnotation.data)
  ...
  public highlight (updateStore: boolean = true): void {
    if (this._isHighlighted) { return }
    if (updateStore) {
      this.view.store.commit('workview/HIGHLIGHT_ANNOTATION', this.id)
    }
  ...
  public select (updateStore: boolean = true): void {
    if (this._isSelected) { return }
    if (updateStore) {
      this.view.store.commit('workview/SELECT_ANNOTATION', this.id)
    }

    this._isSelected = true
  }

  public deselect (updateStore: boolean = true): void {
    if (!this._isSelected) { return }
    ...
    if (updateStore) {
      this.view.store.commit('workview/DESELECT_ANNOTATION', this.id)
    }
  ...
  private constructFromPayload (payload: StageAnnotation): void {
    const type = this.view.editor.resolveAnnotationType(payload)
    if (!type) { throw new Error('Cannot resolve annotation type') }

    const data = this.normalizeAnnotationData(type, payload.data)
  ...
  private constructFromInferenceData (
    ...
    const normalizedData = this.normalizeAnnotationData(type, inferenceData)
  ...
  private cloneInstance (
    ...
    const data = annotation.serializeAnnotationData()
    if (!data) { return }
  ...
  private normalizeAnnotationData (type: string, data: AnnotationData): AnnotationData | null {
    ...
    const serializer = serializerManager.getSerializer(type)
    if (!serializer) { return null }

    if ('frames' in data && 'segments' in data && 'loadedVideo' in this.view) {
  ...
  private normalizeSubAnnotationsData (
    ...
    const { subAnnotationTypes } = this.view
  ...
  private resolveVideoFrameData (
    ...
    const { mainAnnotationTypes } = this.view
  ...
  private serializeStageAnnotation (): StageAnnotationWithOptionalStageId | null {
    const data = this.serializeAnnotationData()
    if (!data) { return null }

    let annotationData = data

    if (
      // this is happening on a video (isVideoAnnotation cannot cover this case)
      this.view.loadedVideo &&
  ...
  private serializeAnnotationData (): StageAnnotationPayload['data'] | null {
    const serializer = this.view.editor.serializerManager.getSerializer(this.type)
    const autoAnnSerializer =
   ...
   */
  public inferVideoData (view: IView = this.view): VideoAnnotationDataPayload {
