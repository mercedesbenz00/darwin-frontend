...
import { C_MAPS } from './consts'
import { loadImageData } from './loadImageData'
import { getWindowLevelsRange } from './windowLevels'
...
const getTransformedImageData = (
  ...
  if (
    (!videoMetadata || !(videoMetadata.type === 'dicom' || videoMetadata.type === 'nifti')) &&
    colorMap === 'default'
  ) {
...
export const resolveTransformedImageData = (
  ...
  if (
    ...
    (
      (videoMetadata?.type === 'dicom' || videoMetadata?.type === 'nifti') &&
      !image.transformedData
    )
  ) {
