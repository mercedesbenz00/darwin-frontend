...
import { Polygon, POLYGON_ANNOTATION_TYPE } from '@/engine/plugins/polygon/types'
...
export function findClosestPolygonEdge (
  ...
  const annotationCursor = editor.camera.canvasViewToImageView(currentPoint)
  let distanceToEdge: Array<[number, ImagePoint, Annotation, number, EditableImagePoint[]]> = []

  const polygonAnnotations =
    editor.activeView.visibleAnnotations.filter(({ type }) => type === POLYGON_ANNOTATION_TYPE)
    ...
    for (const path of paths) {
      for (let i = 0; i < path.length; i++) {
        ...
        const distanceFromCusor = euclideanDistance(
          editor.camera.imageViewToCanvasView(annotationCursor),
          editor.camera.imageViewToCanvasView(pointOnLine)
        )
