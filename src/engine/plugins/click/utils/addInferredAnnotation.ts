...
import { calcCentroid } from '@/engine/graphics'
import { AutoAnnotateData } from '@/engine/models/annotation'
import { ClickerTool } from '@/engine/plugins/click/ClickerTool'
import { Polygon } from '@/engine/plugins/polygon/types'
import { retrievePolygonData } from '@/engine/plugins/polygon/utils/retrievePolygonData'
...
export const addInferredAnnotation = async (
  ...
  const epsilon: number =
    (document as any).darwin_epsilon || context.editor.clickerEpsilon
  ...
  if (currentAnnotation) {
    ...
    clickTool.actionGroup.do({
      do (): Promise<boolean> {
        ...
        context.editor.activeView.annotationManager.persistUpdateAnnotation(updated)
    ...
    const annotation = await context.editor.activeView.prepareAnnotationForCreation(
    ...
    const addAction = addAnnotationAction(context.editor, annotation)
