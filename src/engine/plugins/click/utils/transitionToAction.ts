import { addAnnotationAction, updateAnnotationData } from '@/engine/actions'
...
export const transitionToAction = (clicker: ClickerTool, context: ToolContext) => {
  ...
  if (!context.editor.hasAnnotation(annotation)) { return }
  ...
  const action = (oldData)
    ? updateAnnotationData(context.editor, annotation, oldData, annotation.data)
    : addAnnotationAction(context.editor, annotation)
