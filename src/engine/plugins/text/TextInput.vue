  ...
  async save () {
    ...
    if (this.text === '') {
      await this.editor.actionManager.do(
        removeSubAnnotationAction(this.editor, 'text', this.masterAnnotation)
      )
    } else {
      const annotation =
        this.editor.initializeSubAnnotation('text', this.masterAnnotation, { text: this.text })
      if (!annotation) { return }

      await this.editor.actionManager.do(
        addOrUpdateSubAnnotation(this.editor, annotation, this.masterAnnotation)
      )
    }
