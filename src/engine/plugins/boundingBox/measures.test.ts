...
import { measures } from '@/engine/plugins/boundingBox/measures'
...
beforeEach(() => {
  ...
  data = {
    topLeft: new EditablePoint({ x: 10, y: 10 }),
    topRight: new EditablePoint({ x: 100, y: 10 }),
    bottomRight: new EditablePoint({ x: 100, y: 100 }),
    bottomLeft: new EditablePoint({ x: 10, y: 100 })
  }

  measureOverlayLabel = 'H: 90.00cm W: 90.00cm'
  measureOverlayItems = [{
    center: 'HOR',
    position: expect.objectContaining({ x: 55, y: 100 }),
    value: '90.00',
    unit: 'cm'
  }, {
    center: 'VER',
    position: expect.objectContaining({ x: 100, y: 55 }),
    value: '90.00',
    unit: 'cm'
  }]
})
