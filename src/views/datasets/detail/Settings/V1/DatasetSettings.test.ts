...
it('matches snapshot when authorized to update', () => {
  const wrapper = shallowMount(DatasetSettings, { localVue, mocks, propsData, store, stubs })
  expect(wrapper).toMatchSnapshot()
})

it('redirects when not authorized to update', () => {
  mocks.$can = (): boolean => false
  shallowMount(DatasetSettings, { localVue, mocks, propsData, store, stubs })
...
const model = {
  ...
  templateEditor: 'template-editor-stub',
...
it('correctly saves workforce managers to store, with correct disabled state', async () => {
  ...
  const templateEditor = wrapper.vm.$refs.templateEditor as any
  templateEditor.saveWorkflowTemplate = jest.fn().mockResolvedValue({ data: null })
...
it('omits confirmation dialog if moving to another route when not dirty', async () => {
  ...
  const templateEditor = wrapper.vm.$refs.templateEditor as any
  templateEditor.saveWorkflowTemplate = jest.fn().mockResolvedValue(buildWorkflowTemplatePayload())
...
describe('shows confirmation dialog if you are trying to move to another route when dirty', () => {
  it('when discard clicked, never submits to the backend and moves the next route', async () => {
    ...
    templateEditor.saveWorkflowTemplate =
      jest.fn().mockResolvedValue(buildWorkflowTemplatePayload())
