...
export default class DatasetSettingsV2 extends Vue {
  ...
  mounted (): void {
    if (this.$can('view_full_datasets')) { return }
    this.$router.replace('/')
  }

  confirmSaveBeforeClose (event: BeforeUnloadEvent): string | void {
    if (this.isDirty) {
  ...
  uploadFiles (files: File[], editor: FroalaEditor): Promise<void> | void {
