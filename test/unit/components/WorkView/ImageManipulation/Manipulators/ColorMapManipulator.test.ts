...
import ColorMapManipulator from '@/components/WorkView/ImageManipulation/Manipulators/ColorMapManipulator.vue'
...
it('matches snapshot', () => {
  const wrapper = shallowMount(ColorMapManipulator, { localVue, propsData: { editor }, stubs })
...
it('set editor imageFilter when slider value changes', async () => {
  ...
  const wrapper = shallowMount(ColorMapManipulator, { localVue, propsData: { editor }, stubs })
  await wrapper.find('color-map-dropdown-stub').vm.$emit('input', 'bone')
