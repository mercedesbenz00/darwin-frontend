...
import { loadPublishedModelTemplates } from '@/utils/wind'
...
it('requests auth from backend', async () => {
  await loadPublishedModelTemplates({ teamId: 2 })
...
it('makes an unauthenticated request if no team id', async () => {
  await loadPublishedModelTemplates({ })
  expect(apiPost).not.toHaveBeenCalled()
...
it('returns error if cached auth is wrong', async () => {
  ...
  const response = await loadPublishedModelTemplates({ teamId: 2 })
...
it('sends authenticated request to wind', async () => {
  await loadPublishedModelTemplates({ teamId: 2 })
  expect(windGet).toHaveBeenCalledWith('model_templates?published', { params: { } })
})
