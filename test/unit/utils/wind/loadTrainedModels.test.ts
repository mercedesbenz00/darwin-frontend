...
import { loadTrainedModels } from '@/utils/wind'
...
it('requests auth from backend', async () => {
  await loadTrainedModels({ teamId: 2 })
...
it('returns error if cached auth is wrong', async () => {
  ...
  const response = await loadTrainedModels({ teamId: 2 })
...
it('sends authenticated request to wind', async () => {
  await loadTrainedModels({ teamId: 2, type: 'auto_annotate' })
  expect(windGet).toHaveBeenCalledWith('trained_models', { params: { type: 'auto_annotate' } })

  await loadTrainedModels({ teamId: 2 })
  expect(windGet).toHaveBeenCalledWith('trained_models', { params: { } })
})
