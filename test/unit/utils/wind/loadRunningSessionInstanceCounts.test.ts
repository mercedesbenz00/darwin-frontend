...
import { loadRunningSessionInstanceCounts } from '@/utils/wind'
...
const params: Parameters<typeof loadRunningSessionInstanceCounts>[0] = {
  granularity: 'day',
...
it('requests auth from backend', async () => {
  await loadRunningSessionInstanceCounts(params)
...
it('returns error if cached auth is wrong', async () => {
  ...
  const response = await loadRunningSessionInstanceCounts(params)
...
it('sends authenticated request to wind', async () => {
  await loadRunningSessionInstanceCounts(params)
  expect(windGet).toHaveBeenCalledWith('running_sessions/fake-id/instance_counts', {
    params: {
      granularity: 'day'
    }
  })
...
it('includes optional "from" param in wind request', async () => {
  await loadRunningSessionInstanceCounts({ ...params, from: '2020-01-01T00:00:00' })
  expect(windGet).toHaveBeenCalledWith('running_sessions/fake-id/instance_counts', {
    params: {
      granularity: 'day',
