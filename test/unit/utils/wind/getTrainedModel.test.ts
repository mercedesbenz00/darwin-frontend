...
import { getTrainedModel } from '@/utils/wind'
...
it('requests auth from backend', async () => {
  await getTrainedModel({ modelId: 'fake-id', teamId: 2 })
...
it('returns error if cached auth is wrong', async () => {
  ...
  const response = await getTrainedModel({ modelId: 'fake-id', teamId: 2 })
...
it('sends authenticated request to wind', async () => {
  await getTrainedModel({ modelId: 'fake-id', teamId: 2 })
  expect(windGet).toHaveBeenCalledWith('trained_models/fake-id')
})
