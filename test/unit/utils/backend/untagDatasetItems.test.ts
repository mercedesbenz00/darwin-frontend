...
let payload: Parameters<typeof untagDatasetItems>[0]
...
it('sends request to backend', async () => {
  await untagDatasetItems(payload)
  expect(apiPut).toHaveBeenCalledWith('datasets/5/untag_items', {
...
it('returns response from backend', async () => {
  const response = await untagDatasetItems(payload)
