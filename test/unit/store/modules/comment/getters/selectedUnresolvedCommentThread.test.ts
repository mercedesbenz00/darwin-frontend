...
describe('getter comment/selectedUnresolvedCommentThread', () => {
  ...
  it('returns null when there is no selected comment thread', () => {
    ...
    expect(store.getters['comment/selectedUnresolvedCommentThread']).toBeNull()
  })
  ...
  it('returns selected comment thread', () => {
    ...
    store.commit('comment/SET_THREADS', [commentThread])
    expect(store.getters['comment/selectedUnresolvedCommentThread']).toEqual(commentThread)
  })
})
