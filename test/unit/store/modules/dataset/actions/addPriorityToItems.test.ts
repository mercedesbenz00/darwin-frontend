...
jest.mock('@/utils/backend', () => ({ addPriorityToItems: jest.fn() }))
...
const ACTION = 'dataset/addPriorityToItems'

const payload: Parameters<typeof addPriorityToItems>[1] = {
...
beforeEach(() => {
  jest.spyOn(backend, 'addPriorityToItems').mockResolvedValue(buildAxiosResponse({ data: null }))
...
it('calls correct backend endpoint', async () => {
  ...
  expect(backend.addPriorityToItems).toHaveBeenCalledWith({
...
it('returns error from backend', async () => {
  jest.spyOn(backend, 'addPriorityToItems')
...
it('pushes items to store when all selected', async () => {
  ...
  expect(store.state.dataset.datasetItems).toEqual([
    { ...datasetItems[0], priority: 3 },
    { ...datasetItems[1], priority: 3 }
  ])
})
