...
beforeEach(() => {
  jest.spyOn(backend, 'loadDatasetItems').mockResolvedValue(
    buildAxiosResponse({ data: { items: datasetItems, metadata: {} } })
  )
})
...
it('sends request ', async () => {
  await store.dispatch('workview/searchFilenames', { search: 'keyword' })
...
it('passes in annotation_class_ids', async () => {
  store.commit('workview/SET_DATASET_ITEMS_FILTER', { annotation_class_ids: [1] })
  await store.dispatch('workview/searchFilenames', { datasetId: sfh.id, search: 'keyword' })
...
it('passes in path', async () => {
  store.commit('workview/SET_DATASET_ITEMS_FILTER', { path: '/folder1' })
  await store.dispatch('workview/searchFilenames', { search: 'keyword' })
  expect(backend.loadDatasetItems)
...
it('passes in statuses', async () => {
  store.commit('workview/SET_DATASET_ITEMS_FILTER', { statuses: [DatasetItemStatus.annotate] })
  await store.dispatch('workview/searchFilenames', { datasetId: sfh.id, search: 'keyword' })
  expect(backend.loadDatasetItems)
...
it('passes in types', async () => {
  store.commit('workview/SET_DATASET_ITEMS_FILTER', { types: [DatasetItemType.image] })
  await store.dispatch('workview/searchFilenames', { datasetId: sfh.id, search: 'keyword' })
  expect(backend.loadDatasetItems)
...
it('passes in video_ids', async () => {
  store.commit('workview/SET_DATASET_ITEMS_FILTER', { video_ids: [1] })
  await store.dispatch('workview/searchFilenames', { search: 'keyword' })
  expect(backend.loadDatasetItems)
...
it('passes in workflow_stage_template_ids', async () => {
  store.commit('workview/SET_DATASET_ITEMS_FILTER', { workflow_stage_template_ids: [1] })
  await store.dispatch('workview/searchFilenames', { datasetId: sfh.id, search: 'keyword' })
  expect(backend.loadDatasetItems)
...
it('returns data as string array', async () => {
  const { data } = await store.dispatch('workview/searchFilenames', { search: 'keyword' })
...
it('responds with parsed error on failure', async () => {
  ...
  const { error } = await store.dispatch('workview/searchFilenames', { search: 'keyword' })
