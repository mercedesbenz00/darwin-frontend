...
import { attachPermissionToModel } from '@/store/modules/apiKey/actions/attachPermissionToModel'
...
let payload: StoreActionPayload<typeof attachPermissionToModel>
...
describe('for running session', () => {
  ...
  it('sends request to api', async () => {
    await store.dispatch('apiKey/attachPermissionToModel', payload)
    expect(backend.updateApiKey).toHaveBeenCalledWith({
      apiKeyId: payload.apiKey.id,
      permissions: [...payload.apiKey.permissions, ...permissions]
    })
  })

  it('returns data retrieved from endpoint', async () => {
    const { data } = await store.dispatch('apiKey/attachPermissionToModel', payload)
  ...
  it('returns error', async () => {
    ...
    const response = await store.dispatch('apiKey/attachPermissionToModel', payload)
